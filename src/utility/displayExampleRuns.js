    ...
    for (let i = 0; i <= 2; i++) {
        try {
            ...
            if (exercise.inputType === "map") {
                ...
                if (i === 0) {
                    $('.examples').append(`<p><em>Note that the displayed Map syntax has been simplified for user readability, but would not actually create a Map() properly.</em></p>`);
                }
                ...
                if (exercise.outputType === "map") {
                    ...
                    $('.examples').append(`<li>${exerciseName}${formattedInput} → ${result}</li>`);
                }
                ...
                if (exercise.outputType === "map") {
                    let formattedResult = prettyPrintMap(result);
                    $('.examples').append(`<li>${exerciseName}${exercise.inputs[i]} → ${formattedResult}</li>`);
                }
                else {
